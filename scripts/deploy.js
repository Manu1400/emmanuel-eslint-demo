...
;(async () => {
    ...
    if (ATOKEN) {
        await exec(
            "git fetch --depth=1 https://github.com/Manu1400/emmanuel-eslint-demo.git gh-pages:gh-pages",
        )
    ...
    if (newVersions !== oldVersions) {
        ...
        if (updated) {
            await exec(
                `git push${
                    ATOKEN
                        ? ` https://manu1400:${ATOKEN}@github.com/Manu1400/emmanuel-eslint-demo.git gh-pages:gh-pages`
                        : ""
                }`,
            )
        }
